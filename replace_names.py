...
def is_word(tok: str) -> bool:
    return bool(re.match(r'^[A-Za-zА-Яа-яЁё0-9\-]+$', tok))

# ---------- простая выжимка кандидатов ----------
# 1) готовые триггеры Marvel (рус/англ), 2) собственные имена (последовательности СЗ с прописными),
# 3) спец-термины (камень, щит/гидра, мстители, битва/война и т.д.)
MARVEL_HINTS = [
...
# Типы по контекстным словам (очень грубо, чтобы выбрать шаблон генерации)
...
CTX_EVENT  = [r"\b(битв|войн|вторжен|противостояни|инцидент|осад|восстани)\w*\b"]
...
SUF_EVENT  = [" Война"," Сражение"," Осадa"," Инцидент"," Противостояние"]
...
def generate_fake(name: str, kind: str) -> str:
    if kind=="person": return gen_person()
    if kind=="place":  return gen_place()
    if kind=="org":    return gen_org()
    if kind=="tech":   return gen_tech()
    if kind=="event":  return gen_event()
    return gen_place()
...
def build_mapping_for_folder(in_dir: Path, seed: dict, fuzzy_threshold: float):
    ...
    for p in sorted(in_dir.glob("*.txt")):
        ...
        for cand in cands:
            # нормализуем базу для сравнения
            bk = base_key(cand)
            # пробуем найти уже существующий ключ той же базы (или очень похожий)
            found_key = None
            ...
            if found_key:
                # если в маппинге уже есть база, дублировать не будем
                continue

            # Новый кандидат — определим тип по окрестному контексту (берём +-80 символов вокруг первого вхождения)
...
# Сиквелы: если кандидат оканчивается на номер — та же база + номер
            sequel = re.search(SEQUEL_SUFFIX_RE, cand)
            if sequel:
                base = re.sub(SEQUEL_SUFFIX_RE, '', cand).rstrip()
                # Если для базы уже что-то есть — используем его + тот же номер
                bk2 = base_key(base)
                base_existing = None
                for k in mapping.keys():
                    if base_key(k)==bk2:
    ...
    # добиваем алиасами уникальные «Имя» и «Фамилия»
    aliases = generate_aliases(mapping)
    for k,v in aliases.items():
        mapping.setdefault(k, v)
        # алиасы считаем «производными», в diff можно опустить, если хочешь «чистый» diff по сущностям

    # Проверка согласованности (печатает WARN в stderr)
...
# ---------- ЗАМЕНА ТЕКСТА (та же логика с морфологией, укороченная) ----------
TOKEN_RE = re.compile(r'([A-Za-zА-Яа-яЁё0-9]+|[\-–—]|[^\w\s]|[\s]+)', re.U)
def tokenize(text): return TOKEN_RE.findall(text)
def is_space(tok):  return tok.isspace()
def is_hyphen(tok): return tok in ('-','–','—')
...
def inflect_like(source: str, target_base: str) -> str:
    ...
    if p_src.tag.number: need.add(p_src.tag.number)
    if p_tgt.tag.gender and p_src.tag.gender:
...
def apply_replacement(tokens, pos_list, entry, tail):
    ...
    # собрать разделители как в исходнике
    seps = []
...
def main():
    ...
    seed = json.loads(Path(args.seed_mapping).read_text(encoding="utf-8")) if args.seed_mapping else {}

    mapping, new_pairs = build_mapping_for_folder(in_dir, seed, args.fuzzy_threshold)

    # право на «Железный человек N» → «<его же> N»
    # уже учтено при генерации, плюс базовый ключ «Железный человек» должен быть в словаре
